...
using Version=RuntimeSupport::Version;
...
struct UpdateResponse {Updates::Manifest manifest;std::wstring download,changes;UpdateMode mode=UpdateMode::No;HttpValidators validators;};
...
static std::wstring gSnapshotDownload;
static LaunchedProcess* gRestartApplication=nullptr;

constexpr UINT UpdateUiMessage=WM_APP+40;
constexpr UINT UpdateUiCloseMessage=WM_APP+41;
struct UpdateUiPayload {std::wstring status,detail,progressText;int progress=-1;};
struct UpdateUiState {HANDLE thread=nullptr,ready=nullptr;HWND window=nullptr;std::wstring status,detail,progressText;int progress=-1,pulse=0;bool closeHovered=false;HFONT titleFont=nullptr,statusFont=nullptr,detailFont=nullptr;HICON icon=nullptr;};
static UpdateUiState gUpdateUi;
static std::wstring gUpdateProgressDetail;
static volatile LONG gUpdateCancelled=0;
constexpr int UpdateProgressPixels=404;
static int gLastDownloadPixel=-1;
static unsigned long long gLastDownloadAmount=~0ull;
static bool gFreshInstall=false;
static bool UpdateCancelled(){return InterlockedCompareExchange(&gUpdateCancelled,0,0)!=0;}
static bool CloseButtonHit(HWND window,LPARAM position){RECT client{};GetClientRect(window,&client);int x=(short)LOWORD(position),y=(short)HIWORD(position);return x>=client.right-46&&x<client.right-10&&y>=10&&y<46;}
static LRESULT CALLBACK UpdateUiProc(HWND window,UINT message,WPARAM wParam,LPARAM lParam){
    ...
    if(message==UpdateUiMessage){auto* payload=(UpdateUiPayload*)lParam;ui->status=payload->status;ui->detail=payload->detail;ui->progressText=payload->progressText;ui->progress=payload->progress;delete payload;InvalidateRect(window,nullptr,FALSE);return 0;}
    if(message==UpdateUiCloseMessage){DestroyWindow(window);return 0;}
    if(message==WM_CLOSE){InterlockedExchange(&gUpdateCancelled,1);ui->status=L"Cancelling";ui->detail=L"Cleaning up…";ui->progressText.clear();ui->progress=-1;InvalidateRect(window,nullptr,FALSE);return 0;}
    if(message==WM_MOUSEMOVE){bool hovered=CloseButtonHit(window,lParam);if(hovered!=ui->closeHovered){ui->closeHovered=hovered;InvalidateRect(window,nullptr,FALSE);}TRACKMOUSEEVENT tracking{sizeof(tracking),TME_LEAVE,window,0};TrackMouseEvent(&tracking);return 0;}
    if(message==WM_MOUSELEAVE){ui->closeHovered=false;InvalidateRect(window,nullptr,FALSE);return 0;}
    if(message==WM_LBUTTONUP&&CloseButtonHit(window,lParam)){SendMessageW(window,WM_CLOSE,0,0);return 0;}
    if(message==WM_TIMER){if(wParam==2){KillTimer(window,2);SetWindowPos(window,HWND_NOTOPMOST,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE);return 0;}ui->pulse=(ui->pulse+3)%140;InvalidateRect(window,nullptr,FALSE);return 0;}
    if(message==WM_NCHITTEST){POINT point{(short)LOWORD(lParam),(short)HIWORD(lParam)};ScreenToClient(window,&point);return CloseButtonHit(window,MAKELPARAM(point.x,point.y))?HTCLIENT:HTCAPTION;}
    ...
    if(message==WM_PAINT){PAINTSTRUCT ps{};HDC dc=BeginPaint(window,&ps);RECT client{};GetClientRect(window,&client);HDC memory=CreateCompatibleDC(dc);HBITMAP bitmap=CreateCompatibleBitmap(dc,client.right,client.bottom);HGDIOBJ oldBitmap=SelectObject(memory,bitmap);HBRUSH background=CreateSolidBrush(RGB(18,20,24));FillRect(memory,&client,background);DeleteObject(background);SetBkMode(memory,TRANSPARENT);
        if(ui->icon)DrawIconEx(memory,28,17,ui->icon,28,28,0,nullptr,DI_NORMAL);SetTextColor(memory,RGB(242,244,248));SelectObject(memory,ui->titleFont);RECT title{ui->icon?68L:28L,20,client.right-58,50};DrawTextW(memory,cfg.displayName.c_str(),-1,&title,DT_SINGLELINE|DT_END_ELLIPSIS|DT_NOPREFIX);
        RECT close{client.right-46,10,client.right-10,46};if(ui->closeHovered){HBRUSH hover=CreateSolidBrush(RGB(43,47,55));FillRect(memory,&close,hover);DeleteObject(hover);}HPEN closePen=CreatePen(PS_SOLID,2,ui->closeHovered?RGB(242,244,248):RGB(145,151,162));HGDIOBJ oldPen=SelectObject(memory,closePen);MoveToEx(memory,close.left+12,close.top+12,nullptr);LineTo(memory,close.right-12,close.bottom-12);MoveToEx(memory,close.right-12,close.top+12,nullptr);LineTo(memory,close.left+12,close.bottom-12);SelectObject(memory,oldPen);DeleteObject(closePen);
        ...
        RECT track{28,124,client.right-28,146};HBRUSH trackBrush=CreateSolidBrush(RGB(43,47,55));FillRect(memory,&track,trackBrush);DeleteObject(trackBrush);RECT fill=track;if(ui->progress>=0)fill.right=fill.left+std::min<LONG>(ui->progress,track.right-track.left);else{int width=92;int travel=(track.right-track.left)+width;fill.left=track.left+ui->pulse*travel/140-width;fill.right=fill.left+width;if(fill.left<track.left)fill.left=track.left;if(fill.right>track.right)fill.right=track.right;}HBRUSH accent=CreateSolidBrush(RGB(98,126,255));if(fill.right>fill.left)FillRect(memory,&fill,accent);DeleteObject(accent);if(!ui->progressText.empty()){SetTextColor(memory,RGB(242,244,248));SelectObject(memory,ui->detailFont);RECT progressLabel{track.left+8,track.top,track.right-8,track.bottom};DrawTextW(memory,ui->progressText.c_str(),-1,&progressLabel,DT_SINGLELINE|DT_RIGHT|DT_VCENTER|DT_NOPREFIX);}
...
static DWORD WINAPI UpdateUiThread(void* parameter){auto* ui=(UpdateUiState*)parameter;WNDCLASSEXW wc{};wc.cbSize=sizeof(wc);wc.style=CS_DROPSHADOW;wc.lpfnWndProc=UpdateUiProc;wc.hInstance=GetModuleHandleW(nullptr);wc.hCursor=LoadCursorW(nullptr,IDC_ARROW);wc.lpszClassName=L"OneKBUpdateWindow";RegisterClassExW(&wc);int width=460,height=166,x=(GetSystemMetrics(SM_CXSCREEN)-width)/2,y=(GetSystemMetrics(SM_CYSCREEN)-height)/2;ui->titleFont=CreateFontW(-17,0,0,0,FW_SEMIBOLD,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,DEFAULT_PITCH,L"Segoe UI");ui->statusFont=CreateFontW(-20,0,0,0,FW_SEMIBOLD,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,DEFAULT_PITCH,L"Segoe UI");ui->detailFont=CreateFontW(-14,0,0,0,FW_NORMAL,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,DEFAULT_PITCH,L"Segoe UI");HICON largeIcon=nullptr,smallIcon=nullptr;if(ExtractIconExW(gLauncherPath.c_str(),0,&largeIcon,&smallIcon,1)){ui->icon=smallIcon?smallIcon:largeIcon;if(smallIcon&&largeIcon)DestroyIcon(largeIcon);}ui->window=CreateWindowExW(WS_EX_TOPMOST|WS_EX_APPWINDOW|WS_EX_LAYERED,wc.lpszClassName,L"Updating",WS_POPUP,x,y,width,height,nullptr,nullptr,wc.hInstance,ui);if(ui->window){SetLayeredWindowAttributes(ui->window,0,255,LWA_ALPHA);if(ui->icon){SendMessageW(ui->window,WM_SETICON,ICON_SMALL,(LPARAM)ui->icon);SendMessageW(ui->window,WM_SETICON,ICON_BIG,(LPARAM)ui->icon);}SetWindowRgn(ui->window,CreateRoundRectRgn(0,0,width+1,height+1,14,14),TRUE);SetTimer(ui->window,1,24,nullptr);SetTimer(ui->window,2,1000,nullptr);ShowWindow(ui->window,SW_SHOW);SetForegroundWindow(ui->window);UpdateWindow(ui->window);}SetEvent(ui->ready);MSG message{};while(ui->window&&GetMessageW(&message,nullptr,0,0)>0){TranslateMessage(&message);DispatchMessageW(&message);}if(ui->icon)DestroyIcon(ui->icon);if(ui->titleFont)DeleteObject(ui->titleFont);if(ui->statusFont)DeleteObject(ui->statusFont);if(ui->detailFont)DeleteObject(ui->detailFont);ui->window=nullptr;return 0;}
static void ShowUpdateUi(const std::wstring& current){if(gUpdateUi.thread)return;InterlockedExchange(&gUpdateCancelled,0);gLastDownloadPixel=-1;gLastDownloadAmount=~0ull;gUpdateUi.status=current.empty()?L"Finding the latest version":L"Checking for updates";gUpdateUi.detail=current.empty()?L"":L"Installed version "+current;gUpdateUi.progress=-1;gUpdateUi.ready=CreateEventW(nullptr,TRUE,FALSE,nullptr);if(!gUpdateUi.ready)return;gUpdateUi.thread=CreateThread(nullptr,0,UpdateUiThread,&gUpdateUi,0,nullptr);if(gUpdateUi.thread)WaitForSingleObject(gUpdateUi.ready,3000);CloseHandle(gUpdateUi.ready);gUpdateUi.ready=nullptr;}
static void SetUpdateUi(const std::wstring& status,const std::wstring& detail,int progress=-1,const std::wstring& progressText=L""){if(!UpdateCancelled()&&gUpdateUi.window){auto* payload=new UpdateUiPayload{status,detail,progressText,progress};if(!PostMessageW(gUpdateUi.window,UpdateUiMessage,0,(LPARAM)payload))delete payload;}}
...
static void UpdateUiDownloadProgress(unsigned long long received,unsigned long long total){if(!gUpdateUi.window)return;unsigned long long amount;if(!total){amount=received/(1024*1024);if(amount==gLastDownloadAmount)return;gLastDownloadAmount=amount;SetUpdateUi(L"Downloading",gUpdateProgressDetail,-1);return;}int pixel=(int)(received*UpdateProgressPixels/total);if(pixel>UpdateProgressPixels)pixel=UpdateProgressPixels;unsigned long long remaining=received<total?total-received:0,remainingMb=(remaining+1024*1024-1)/(1024*1024);if(pixel==gLastDownloadPixel&&remainingMb==gLastDownloadAmount)return;gLastDownloadPixel=pixel;gLastDownloadAmount=remainingMb;std::wstring label;if(remainingMb)label=std::to_wstring(remainingMb)+L" MB";SetUpdateUi(L"Downloading",gUpdateProgressDetail,pixel,label);}
...
static bool ValidVersion(const std::wstring& value,Version* result=nullptr){return RuntimeSupport::ParseVersion(value,result);}
...
static bool AtomicWriteLine(const std::wstring& path,const std::wstring& value){return RuntimeSupport::AtomicWriteUtf8(path,value+L"\n");}
...
static bool ExecutableSubsystem(const std::wstring& path,WORD& subsystem){return RuntimeSupport::ExecutableSubsystem(path,subsystem,true);}
...
static bool SameFileBytes(const std::wstring& a,const std::wstring& b){std::vector<char> x,y;return ReadBytes(a,Limits::MaxLauncherSize,x)&&ReadBytes(b,Limits::MaxLauncherSize,y)&&x==y;}
...
static bool ValidDisplayName(const std::wstring& value){if(value.empty()||value.size()>240||value.back()==L'.'||value.back()==L' ')return false;for(wchar_t c:value)if(c<L' '||wcschr(L"<>:\"/\\|?*",c))return false;return true;}
static bool ParseUpdateState(const std::vector<char>& bytes,UpdateMode& mode,HttpValidators& validators,std::wstring* download=nullptr,std::wstring* name=nullptr){mode=UpdateMode::No;validators=HttpValidators{};if(download)download->clear();if(name)name->clear();std::set<std::string> seen;return !bytes.empty()&&RuntimeSupport::ParseKeyValues(bytes,Limits::MaxUpdateConfigurationSize,[&](const std::string& key,const std::wstring& value){if(!seen.insert(key).second)return false;if(key=="name"&&name){*name=value;return ValidDisplayName(value);}if(key=="updates")return ParseMode(value,mode);if(key=="etag")return ValidHttpValidator(validators.etag=value);if(key=="last_modified")return ValidHttpValidator(validators.lastModified=value);if(key=="download"&&download){*download=value;return Updates::Reference(value);}return false;});}
static bool AtomicWriteUtf8(const std::wstring& path,const std::wstring& value);
static bool WriteUpdateState(const std::wstring& path,bool includeMode,UpdateMode mode,const HttpValidators& validators,const std::wstring& download=L"",const std::wstring& name=L""){std::wstring text;if(includeMode&&ValidDisplayName(name))text=L"name="+name+L"\n";if(includeMode&&mode!=UpdateMode::No)text+=L"updates="+std::wstring(ModeName(mode))+L"\n";if(!download.empty())text+=L"download="+download+L"\n";if(ValidHttpValidator(validators.etag))text+=L"etag="+validators.etag+L"\n";if(ValidHttpValidator(validators.lastModified))text+=L"last_modified="+validators.lastModified+L"\n";if(text.empty())return DeleteFileW(path.c_str())||GetLastError()==ERROR_FILE_NOT_FOUND;return AtomicWriteUtf8(path,text);}
static void LoadPersistedOverrides(){cfg.displayName=cfg.fallbackDisplayName;cfg.mode=UpdateMode::No;gApplicationValidators=HttpValidators{};gSnapshotDownload.clear();DWORD lock=gConfigurationMutex?WaitForSingleObject(gConfigurationMutex,INFINITE):WAIT_OBJECT_0;bool owns=lock==WAIT_OBJECT_0||lock==WAIT_ABANDONED;std::vector<char> bytes;UpdateMode mode=UpdateMode::No;HttpValidators validators;std::wstring name;if(owns&&ReadBytes(g.updateState,Limits::MaxUpdateConfigurationSize,bytes)&&ParseUpdateState(bytes,mode,validators,&gSnapshotDownload,&name)){cfg.mode=mode;gApplicationValidators=validators;if(!name.empty())cfg.displayName=name;}if(owns&&gConfigurationMutex)ReleaseMutex(gConfigurationMutex);}
static bool PersistApplicationState(){DWORD lock=gConfigurationMutex?WaitForSingleObject(gConfigurationMutex,INFINITE):WAIT_OBJECT_0;bool owns=lock==WAIT_OBJECT_0||lock==WAIT_ABANDONED;bool ok=owns&&WriteUpdateState(g.updateState,true,cfg.mode,gApplicationValidators,gSnapshotDownload,cfg.displayName);if(owns&&gConfigurationMutex)ReleaseMutex(gConfigurationMutex);return ok;}
...
using HttpBody=bool (*)(void*,const unsigned char*,unsigned);
static bool HttpGetUrl(const std::wstring& initial,std::vector<char>* memory,const std::wstring* file,unsigned long long cap,DWORD timeout,const wchar_t* headers=nullptr,HttpValidators* responseValidators=nullptr,HttpBody consume=nullptr,void* consumeContext=nullptr){
    ...
    for(unsigned redirect=0;redirect<=5&&!UpdateCancelled();++redirect){URL_COMPONENTS u{};u.dwStructSize=sizeof(u);u.dwHostNameLength=(DWORD)-1;u.dwUrlPathLength=(DWORD)-1;u.dwExtraInfoLength=(DWORD)-1;u.dwUserNameLength=(DWORD)-1;u.dwPasswordLength=(DWORD)-1;if(!WinHttpCrackUrl(requestUrl.c_str(),0,0,&u)||!u.dwHostNameLength||u.dwUserNameLength||u.dwPasswordLength)break;std::wstring host(u.lpszHostName,u.dwHostNameLength),object(u.lpszUrlPath,u.dwUrlPathLength);bool secure=u.nScheme==INTERNET_SCHEME_HTTPS;if((!secure&&u.nScheme!=INTERNET_SCHEME_HTTP)||(!secure&&!IsLocalHost(host)))break;if(u.dwExtraInfoLength)object.append(u.lpszExtraInfo,u.dwExtraInfoLength);const wchar_t* connectHost=_wcsicmp(host.c_str(),L"localhost")==0?L"127.0.0.1":host.c_str();HINTERNET connect=WinHttpConnect(session,connectHost,u.nPort,0);HINTERNET request=connect?WinHttpOpenRequest(connect,L"GET",object.c_str(),nullptr,WINHTTP_NO_REFERER,WINHTTP_DEFAULT_ACCEPT_TYPES,secure?WINHTTP_FLAG_SECURE:0):nullptr;DWORD disable=WINHTTP_DISABLE_REDIRECTS;if(request&&!WinHttpSetOption(request,WINHTTP_OPTION_DISABLE_FEATURE,&disable,sizeof(disable))){WinHttpCloseHandle(request);request=nullptr;}bool received=request&&WinHttpSendRequest(request,headers,headers?(DWORD)-1L:0,WINHTTP_NO_REQUEST_DATA,0,0,0)&&WinHttpReceiveResponse(request,nullptr);DWORD status=0,n=sizeof(status);if(received)received=WinHttpQueryHeaders(request,WINHTTP_QUERY_STATUS_CODE|WINHTTP_QUERY_FLAG_NUMBER,nullptr,&status,&n,nullptr)!=FALSE;
        ...
        if(received&&status==200){DWORD lengthBytes=0;WinHttpQueryHeaders(request,WINHTTP_QUERY_CONTENT_LENGTH,nullptr,nullptr,&lengthBytes,nullptr);std::vector<wchar_t> lengthText(lengthBytes/sizeof(wchar_t)+1);bool hasLength=lengthBytes&&WinHttpQueryHeaders(request,WINHTTP_QUERY_CONTENT_LENGTH,nullptr,lengthText.data(),&lengthBytes,nullptr)!=FALSE;wchar_t* lengthEnd=nullptr;unsigned long long declared=hasLength?_wcstoui64(lengthText.data(),&lengthEnd,10):0;hasLength=hasLength&&lengthEnd&&!*lengthEnd&&declared;if(hasLength&&declared>cap){WinHttpCloseHandle(request);if(connect)WinHttpCloseHandle(connect);break;}unsigned long long total=0;char block[32768];ok=true;for(;;){if(UpdateCancelled()){ok=false;break;}DWORD got=0;if(!WinHttpReadData(request,block,sizeof(block),&got)){ok=false;break;}if(!got)break;total+=got;if(total>cap){ok=false;break;}if(memory)memory->insert(memory->end(),block,block+got);else if(consume){if(!consume(consumeContext,(const unsigned char*)block,got)){ok=false;break;}UpdateUiDownloadProgress(total,hasLength?declared:0);}else{DWORD wrote=0;if(!WriteFile(output,block,got,&wrote,nullptr)||wrote!=got){ok=false;break;}if(file)UpdateUiDownloadProgress(total,hasLength?declared:0);}}if(ok&&responseValidators){auto readValidator=[&](DWORD query,std::wstring& destination){DWORD bytes=0;WinHttpQueryHeaders(request,query,nullptr,nullptr,&bytes,nullptr);if(!bytes||bytes>513*sizeof(wchar_t))return;std::vector<wchar_t> value(bytes/sizeof(wchar_t)+1);if(WinHttpQueryHeaders(request,query,nullptr,value.data(),&bytes,nullptr)){std::wstring parsed(value.data());if(ValidHttpValidator(parsed))destination=parsed;}};readValidator(WINHTTP_QUERY_ETAG,responseValidators->etag);readValidator(WINHTTP_QUERY_LAST_MODIFIED,responseValidators->lastModified);}}
        else if(received)gLastHttpStatus=status;
        if(request)WinHttpCloseHandle(request);if(connect)WinHttpCloseHandle(connect);break;
    }WinHttpCloseHandle(session);if(output!=INVALID_HANDLE_VALUE){CloseHandle(output);if(!ok)DeleteFileW(file->c_str());}return ok;
}
#ifndef ONEKB_RUNTIME_ONLY
...
static bool ParseRemoteConfiguration(const std::vector<char>& bytes,const std::wstring& base,UpdateResponse& response){Updates::Manifest manifest;if(!Updates::Parse(bytes,manifest))return false;std::wstring download,changes;if(!ResolveDownloadUrl(manifest.download,base,download)||(!manifest.changes.empty()&&!ResolveDownloadUrl(manifest.changes,base,changes)))return false;bool encrypted=gLauncherOverlay.encryption==PayloadEncryption::Launcher;if(EndsWithInsensitive(download,encrypted?L".1KB":L".zip")==false||(!changes.empty()&&!EndsWithInsensitive(changes,encrypted?L".1KB":L".zip")))return false;HttpValidators validators=response.validators;response=UpdateResponse{};response.manifest=manifest;response.download=download;response.changes=changes;response.validators=validators;return ParseMode(manifest.updates,response.mode);}
...
static bool AtomicWriteUtf8(const std::wstring& path,const std::wstring& value){return RuntimeSupport::AtomicWriteUtf8(path,value);}
...
static void DeleteTree(const std::wstring& path){RuntimeSupport::DeleteTree(path);}
...
static void Cleanup(const std::wstring& current){
    WIN32_FIND_DATAW f{};HANDLE h=FindFirstFileW((g.versions+L"\\*").c_str(),&f);if(h==INVALID_HANDLE_VALUE)return;do{std::wstring n=f.cFileName,path=g.versions+L"\\"+n;bool directory=(f.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY)!=0;if(directory&&!(f.dwFileAttributes&FILE_ATTRIBUTE_REPARSE_POINT)&&n!=L"."&&n!=L".."&&((n!=current&&ValidVersion(n))||n==L".update")&&!DirectoryRunning(path))DeleteTree(path);else if(!directory&&(n.rfind(L"current.txt.tmp.",0)==0||n.rfind(L"1kb.ini.tmp.",0)==0||n.rfind(L"launcher.tmp.",0)==0))DeleteFileW(path.c_str());}while(FindNextFileW(h,&f));FindClose(h);
}
...
static bool StopTrackedApplication(LaunchedProcess& launched);
static std::wstring PackageContext(const std::wstring& url){size_t slash=url.find_last_of(L'/');return slash==url.npos?url:url.substr(slash+1);}
struct PackageDownload {Updates::StreamExtractor* extractor=nullptr;PayloadDecryptStream* decrypt=nullptr;bool encrypted=false;};
static bool ExtractPackageBytes(void* context,const uint8_t* bytes,unsigned count){return ((Updates::StreamExtractor*)context)->consume(bytes,count);}
static bool ConsumePackageDownload(void* context,const unsigned char* bytes,unsigned count){auto& download=*(PackageDownload*)context;return download.encrypted?DecryptPayloadBytes(*download.decrypt,bytes,count):download.extractor->consume(bytes,count);}
static bool PreparePackage(const std::wstring& url,const std::wstring& prepared,bool changes,std::set<std::wstring>& deleted,std::wstring& error){
    if(!Updates::Tree(prepared)){error=L"The package staging directory could not be created.";return false;}bool encrypted=gLauncherOverlay.encryption==PayloadEncryption::Launcher;Updates::StreamExtractor extractor(prepared);PayloadDecryptStream decrypt;PayloadCryptoError crypto;PackageDownload download{&extractor,&decrypt,encrypted};if(encrypted&&!BeginPayloadDecrypt(decrypt,gLauncherOverlay.secret,cfg.identity.appId,PackageContext(url),ExtractPackageBytes,&extractor,&crypto)){error=crypto.message;return false;}gLastDownloadPixel=-1;gLastDownloadAmount=~0ull;SetUpdateUi(L"Starting download",gUpdateProgressDetail,-1);gLastHttpStatus=0;
    bool received=HttpGetUrl(url,nullptr,nullptr,Updates::PackageLimit,Limits::DownloadTimeoutMs,nullptr,nullptr,ConsumePackageDownload,&download);bool authenticated=!encrypted||received&&FinishPayloadDecrypt(decrypt);bool ok=received&&authenticated&&extractor.finish();EndPayloadDecrypt(decrypt);if(ok){auto instructions=prepared+L"\\1kb.updates.ini";ok=changes?Updates::ReadUpdateInstructions(prepared,deleted):Updates::Missing(instructions);}if(!ok)error=UpdateCancelled()?L"Cancelled.":!crypto.message.empty()?crypto.message:gLastHttpStatus?L"The application server returned HTTP "+std::to_wstring(gLastHttpStatus)+L" while downloading the update.":received?L"The streaming package is invalid, unsafe, truncated, or malformed.":L"The application package could not be downloaded.";return ok;
}
static bool PrepareAndActivate(const UpdateResponse& update,bool,std::wstring& error){
    std::wstring current;bool haveCurrent=CurrentValid(current),sameSnapshot=haveCurrent&&!gSnapshotDownload.empty()&&gSnapshotDownload==update.manifest.download;
    bool incremental=Updates::UseChanges(haveCurrent,gSnapshotDownload,update.manifest);if(sameSnapshot&&!incremental){error=L"The release does not contain changes for this snapshot.";return false;}
    auto work=g.root+L"\\.update";RuntimeSupport::DeleteTree(work);auto snapshot=work+L"\\snapshot",changes=work+L"\\changes";if(!Updates::Tree(work)){error=L"The update work directory could not be created.";return false;}
    std::set<std::wstring> deleted;bool prepared=false;
    if(incremental)prepared=PreparePackage(update.changes,changes,true,deleted,error);
    else{prepared=PreparePackage(update.download,snapshot,false,deleted,error);if(prepared&&!update.changes.empty())prepared=PreparePackage(update.changes,changes,true,deleted,error);}
    if(!prepared){RuntimeSupport::DeleteTree(work);return false;}
    HANDLE gate=CreateMutexW(nullptr,FALSE,ObjectName(L"_LauncherLaunch").c_str());DWORD gateLock=gate?WaitForSingleObject(gate,INFINITE):WAIT_FAILED;if(gateLock!=WAIT_OBJECT_0&&gateLock!=WAIT_ABANDONED){if(gate)CloseHandle(gate);RuntimeSupport::DeleteTree(work);error=L"The launcher gate could not be acquired.";return false;}
    if(gRestartApplication&&gRestartApplication->handle&&!StopTrackedApplication(*gRestartApplication)){ReleaseMutex(gate);CloseHandle(gate);RuntimeSupport::DeleteTree(work);error=L"The running application could not be stopped.";return false;}
    while(haveCurrent&&DirectoryRunning(g.versions+L"\\"+current)){if(UpdateCancelled()){ReleaseMutex(gate);CloseHandle(gate);RuntimeSupport::DeleteTree(work);error=L"Cancelled.";return false;}SetUpdateUi(L"Waiting for application to close",gUpdateProgressDetail,-1);Sleep(500);}
    SetUpdateUi(L"Installing",gUpdateProgressDetail,-1);gResolvedVersion.clear();gResolvedApplication.clear();bool installed=incremental?Updates::InstallIncremental(g.root,current,update.manifest.version,changes,deleted):Updates::InstallFull(g.root,update.manifest.version,snapshot,update.changes.empty()?nullptr:&changes,deleted);
    if(installed&&AppPath(update.manifest.version).empty()){DeleteFileW(g.current.c_str());installed=false;error=L"The installed release does not have an unambiguous application executable.";}
    if(installed){gSnapshotDownload=update.manifest.download;if(!PersistApplicationState())WarnPersistence();}
    ReleaseMutex(gate);CloseHandle(gate);RuntimeSupport::DeleteTree(work);if(!installed&&error.empty())error=L"The update could not be installed; the next launch will repair from the full snapshot.";return installed;
}
enum class UpdateResult { Failed,NoUpdate,Installed,Cancelled };
static UpdateResult CheckAndInstall(std::wstring& installed,std::wstring& error,bool restarting=false){installed.clear();error.clear();std::wstring current;bool haveCurrent=CurrentValid(current);gFreshInstall=!haveCurrent;UpdateResponse response;ResolveResult resolved=ResolveRemote(haveCurrent,response);if(resolved==ResolveResult::NotModified){CloseUpdateUi();return UpdateResult::NoUpdate;}if(resolved==ResolveResult::Failed){if(UpdateCancelled()){error=L"Cancelled.";return UpdateResult::Cancelled;}error=gLastHttpStatus?L"The application source server returned HTTP "+std::to_wstring(gLastHttpStatus)+L" while resolving the application manifest.":L"The application source URL could not be resolved to a valid application manifest.";return UpdateResult::Failed;}if(haveCurrent&&response.manifest.version==current){CommitApplicationValidators(response.validators);CloseUpdateUi();return UpdateResult::NoUpdate;}if(haveCurrent&&!Updates::Newer(response.manifest.version,current)){error=L"The application manifest does not contain a newer version.";return UpdateResult::Failed;}if(!gUpdateUi.thread&&(restarting||cfg.mode==UpdateMode::Yes))ShowUpdateUi(current);gUpdateProgressDetail=gFreshInstall?L"Version "+response.manifest.version:L"Version "+response.manifest.version+L"  |  Installed "+current;SetUpdateUi(gFreshInstall?L"Latest version found":L"Update available",gUpdateProgressDetail,0);if(!PrepareAndActivate(response,true,error))return UpdateCancelled()?UpdateResult::Cancelled:UpdateResult::Failed;gApplicationValidators=response.validators;if(!PersistApplicationState())WarnPersistence();installed=response.manifest.version;return UpdateResult::Installed;}
static BOOL CALLBACK RequestProcessWindowClose(HWND window,LPARAM processValue){DWORD process=0;GetWindowThreadProcessId(window,&process);if(process==(DWORD)processValue)PostMessageW(window,WM_CLOSE,0,0);return TRUE;}
static bool StopTrackedApplication(LaunchedProcess& launched){if(!launched.handle)return false;DWORD code=0;if(!GetExitCodeProcess(launched.handle,&code))return false;if(code==STILL_ACTIVE){EnumWindows(RequestProcessWindowClose,(LPARAM)launched.id);DWORD wait=WaitForSingleObject(launched.handle,Limits::GracefulRestartTimeoutMs);if(wait!=WAIT_OBJECT_0&&(!TerminateProcess(launched.handle,0)||WaitForSingleObject(launched.handle,Limits::ForcedRestartTimeoutMs)!=WAIT_OBJECT_0))return false;}CloseHandle(launched.handle);launched.handle=nullptr;launched.id=0;return true;}
static bool RestartTrackedApplication(LaunchedProcess& launched,const std::wstring& installed){return (!launched.handle||StopTrackedApplication(launched))&&StartApplication(installed,&launched);}
...
#ifdef ONEKB_RUNTIME_ONLY
static std::wstring InstalledLauncherPath(){return g.root+L"\\launcher.exe";}
static std::wstring UninstallKey(){return L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\"+cfg.identity.mutexKey;}
static bool ShortcutPath(std::wstring& path){PWSTR programs=nullptr;if(FAILED(SHGetKnownFolderPath(FOLDERID_Programs,KF_FLAG_CREATE,nullptr,&programs)))return false;path=std::wstring(programs)+L"\\"+cfg.displayName+L".lnk";CoTaskMemFree(programs);return ValidDisplayName(cfg.displayName);}
static bool InstallLauncher(){std::wstring installed=InstalledLauncherPath();if(!_wcsicmp(gLauncherPath.c_str(),installed.c_str())||(Exists(installed)&&SameFileBytes(gLauncherPath,installed)))return Exists(installed);std::wstring temporary=g.root+L"\\launcher.tmp."+std::to_wstring(GetCurrentProcessId());DeleteFileW(temporary.c_str());bool ok=CopyFileW(gLauncherPath.c_str(),temporary.c_str(),FALSE)!=FALSE&&MoveFileExW(temporary.c_str(),installed.c_str(),MOVEFILE_REPLACE_EXISTING|MOVEFILE_WRITE_THROUGH)!=FALSE;if(!ok)DeleteFileW(temporary.c_str());return ok;}
static ULONGLONG InstalledBytes(const std::wstring& directory){ULONGLONG total=0;WIN32_FIND_DATAW f{};HANDLE search=FindFirstFileW((directory+L"\\*").c_str(),&f);if(search==INVALID_HANDLE_VALUE)return 0;do{if(!wcscmp(f.cFileName,L".")||!wcscmp(f.cFileName,L".."))continue;std::wstring path=directory+L"\\"+f.cFileName;if(f.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY){if(!(f.dwFileAttributes&FILE_ATTRIBUTE_REPARSE_POINT))total+=InstalledBytes(path);}else total+=((ULONGLONG)f.nFileSizeHigh<<32)|f.nFileSizeLow;}while(FindNextFileW(search,&f));FindClose(search);return total;}
static void RegisterWindowsIntegration(const std::wstring& version){if(!InstallLauncher())return;std::wstring launcher=InstalledLauncherPath(),shortcut;if(ShortcutPath(shortcut)){HRESULT initialized=CoInitializeEx(nullptr,COINIT_APARTMENTTHREADED);IShellLinkW* link=nullptr;IPersistFile* file=nullptr;if(SUCCEEDED(CoCreateInstance(CLSID_ShellLink,nullptr,CLSCTX_INPROC_SERVER,IID_PPV_ARGS(&link)))&&SUCCEEDED(link->SetPath(launcher.c_str()))&&SUCCEEDED(link->SetWorkingDirectory(g.root.c_str()))&&SUCCEEDED(link->SetIconLocation(launcher.c_str(),0))&&SUCCEEDED(link->QueryInterface(IID_PPV_ARGS(&file))))file->Save(shortcut.c_str(),TRUE);if(file)file->Release();if(link)link->Release();if(SUCCEEDED(initialized))CoUninitialize();}
    HKEY key=nullptr;if(RegCreateKeyExW(HKEY_CURRENT_USER,UninstallKey().c_str(),0,nullptr,0,KEY_SET_VALUE,nullptr,&key,nullptr)==ERROR_SUCCESS){auto stringValue=[&](const wchar_t* name,const std::wstring& value){RegSetValueExW(key,name,0,REG_SZ,(const BYTE*)value.c_str(),(DWORD)((value.size()+1)*sizeof(wchar_t)));};DWORD one=1;ULONGLONG kilobytes=(InstalledBytes(g.root)+1023)/1024;DWORD estimatedSize=(DWORD)(kilobytes>MAXDWORD?MAXDWORD:kilobytes);stringValue(L"DisplayName",cfg.displayName);stringValue(L"DisplayVersion",version);stringValue(L"InstallLocation",g.root);stringValue(L"DisplayIcon",L"\""+launcher+L"\",0");stringValue(L"UninstallString",L"\""+launcher+L"\" __1KB_UNINSTALL__");RegSetValueExW(key,L"EstimatedSize",0,REG_DWORD,(const BYTE*)&estimatedSize,sizeof(estimatedSize));RegSetValueExW(key,L"NoModify",0,REG_DWORD,(const BYTE*)&one,sizeof(one));RegSetValueExW(key,L"NoRepair",0,REG_DWORD,(const BYTE*)&one,sizeof(one));RegCloseKey(key);}}
static void RemoveWindowsIntegration(){std::wstring shortcut;if(ShortcutPath(shortcut))DeleteFileW(shortcut.c_str());RegDeleteTreeW(HKEY_CURRENT_USER,UninstallKey().c_str());}
static bool ExactUninstallInvocation(){int count=0;LPWSTR* values=CommandLineToArgvW(GetCommandLineW(),&count);bool requested=values&&count==2&&!wcscmp(values[1],L"__1KB_UNINSTALL__");if(values)LocalFree(values);return requested;}
static bool StopInvokingInstalledLauncher(){std::wstring installed=InstalledLauncherPath();if(_wcsicmp(gLauncherPath.c_str(),installed.c_str()))return true;DWORD parentId=0;HANDLE snapshot=CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS,0);if(snapshot!=INVALID_HANDLE_VALUE){PROCESSENTRY32W entry{};entry.dwSize=sizeof(entry);if(Process32FirstW(snapshot,&entry))do{if(entry.th32ProcessID==GetCurrentProcessId()){parentId=entry.th32ParentProcessID;break;}}while(Process32NextW(snapshot,&entry));CloseHandle(snapshot);}if(!parentId)return true;HANDLE parent=OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION|PROCESS_TERMINATE|SYNCHRONIZE,FALSE,parentId);if(!parent)return true;wchar_t path[32768]{};DWORD size=_countof(path);bool ours=QueryFullProcessImageNameW(parent,0,path,&size)&&!_wcsicmp(path,installed.c_str());bool ok=!ours||(TerminateProcess(parent,0)&&WaitForSingleObject(parent,Limits::ForcedRestartTimeoutMs)==WAIT_OBJECT_0);CloseHandle(parent);return ok;}
static int UninstallMain(){if(!InitPaths())return 1;HANDLE update=CreateMutexW(nullptr,FALSE,ObjectName(L"_LauncherUpdate").c_str()),launch=CreateMutexW(nullptr,FALSE,ObjectName(L"_LauncherLaunch").c_str());if(!update||!launch){if(update)CloseHandle(update);if(launch)CloseHandle(launch);return 1;}DWORD updateLock=WaitForSingleObject(update,INFINITE),launchLock=(updateLock==WAIT_OBJECT_0||updateLock==WAIT_ABANDONED)?WaitForSingleObject(launch,INFINITE):WAIT_FAILED;bool ownsUpdate=updateLock==WAIT_OBJECT_0||updateLock==WAIT_ABANDONED,ownsLaunch=launchLock==WAIT_OBJECT_0||launchLock==WAIT_ABANDONED,ok=ownsUpdate&&ownsLaunch;
    gConfigurationMutex=CreateMutexW(nullptr,FALSE,ObjectName(L"_LauncherConfiguration").c_str());if(gConfigurationMutex)LoadPersistedOverrides();std::wstring current;if(ok&&CurrentValid(current)&&DirectoryRunning(g.versions+L"\\"+current)){LaunchedProcess running;ok=TrackRunningApplication(AppPath(current),current,running)&&StopTrackedApplication(running);}if(ok)ok=StopInvokingInstalledLauncher();for(unsigned i=0;ok&&DirectoryRunning(g.root)&&i<50;++i)Sleep(100);if(ok&&DirectoryRunning(g.root))ok=false;if(ok){RemoveWindowsIntegration();ok=RuntimeSupport::DeleteTree(g.root);}if(ownsLaunch)ReleaseMutex(launch);if(ownsUpdate)ReleaseMutex(update);CloseHandle(launch);CloseHandle(update);if(gConfigurationMutex){CloseHandle(gConfigurationMutex);gConfigurationMutex=nullptr;}if(!ok)MessageBoxW(nullptr,L"The application is still running or its installation could not be removed.",cfg.displayName.c_str(),MB_ICONERROR);return ok?0:1;}
...
#endif
    gLastHttpStatus=0;bool responseOk=HttpGetUrl(updateUrl,nullptr,&downloaded,Limits::MaxLauncherSize,Limits::DownloadTimeoutMs,headers.empty()?nullptr:headers.c_str(),&responseValidators);if(!responseOk&&gLastHttpStatus==304){DeleteFileW(downloaded.c_str());ReleaseMutex(mutex);CloseHandle(mutex);return;}
...
static int BackgroundUpdateMain(){if(!InitPaths())return 1;HANDLE updateMutex=CreateMutexW(nullptr,FALSE,ObjectName(L"_LauncherUpdate").c_str());if(!updateMutex)return 1;gConfigurationMutex=CreateMutexW(nullptr,FALSE,ObjectName(L"_LauncherConfiguration").c_str());if(!gConfigurationMutex){CloseHandle(updateMutex);return 1;}DWORD lock=WaitForSingleObject(updateMutex,INFINITE);if(lock==WAIT_OBJECT_0||lock==WAIT_ABANDONED){LoadPersistedOverrides();std::wstring installed,error;UpdateResult result=CheckAndInstall(installed,error);std::wstring current;if(CurrentValid(current)){Cleanup(current);if(result==UpdateResult::Installed)RegisterWindowsIntegration(current);}UpdateLauncherRuntime();ReleaseMutex(updateMutex);}CloseHandle(gConfigurationMutex);gConfigurationMutex=nullptr;CloseHandle(updateMutex);return 0;}
#else
static bool StartRuntimeUpdate(){return false;}
static bool StartBackgroundUpdate(){return false;}
static void RegisterWindowsIntegration(const std::wstring&){}
#endif
...
static bool ReadWholeFile(const std::wstring& path,std::vector<uint8_t>& out){std::vector<char> bytes;if(!ReadBytes(path,Limits::MaxLauncherSize,bytes))return false;out.assign(bytes.begin(),bytes.end());return true;}
...
bool LauncherAppRelativeDirectory(const std::wstring& appId,std::wstring& relativeDirectory){AppIdentity identity;if(!ParseAppId(appId,identity)||identity.appId!=appId)return false;relativeDirectory=identity.relativeDirectory;return true;}
...
static int LauncherMain(){
    ...
    DWORD updateLock=WaitForSingleObject(updateMutex,0);bool ownsUpdate=updateLock==WAIT_OBJECT_0||updateLock==WAIT_ABANDONED;
    if(updateFirst&&!ownsUpdate){CloseHandle(gConfigurationMutex);gConfigurationMutex=nullptr;CloseHandle(updateMutex);return updateLock==WAIT_TIMEOUT?0:1;}
    if(ownsUpdate)LoadPersistedOverrides();
    if(updateFirst)ShowUpdateUi(hadInstalled?existing:L"");
    ...
    if(gUpdateUi.thread&&result==UpdateResult::Installed)SetUpdateUi(hadInstalled?L"Update ready":L"Ready to launch",L"Version "+installed+L" is installed",UpdateProgressPixels);else if(result==UpdateResult::Failed)ShowUpdateFailureUi(updateError,hadInstalled);
    if(result==UpdateResult::Cancelled&&!hadInstalled){CloseUpdateUi();if(ownsUpdate)ReleaseMutex(updateMutex);CloseHandle(gConfigurationMutex);gConfigurationMutex=nullptr;CloseHandle(updateMutex);return 0;}
    std::wstring ready;if(result==UpdateResult::Installed&&CurrentValid(ready))RegisterWindowsIntegration(ready);
    ...
    if(!updateFirst){
        ...
        else{UpdateMode operationMode=cfg.mode;gRestartApplication=operationMode==UpdateMode::Restart?&launched:nullptr;result=CheckAndInstall(installed,updateError,operationMode==UpdateMode::Restart);gRestartApplication=nullptr;
            if(result==UpdateResult::Installed&&operationMode==UpdateMode::Restart){SetUpdateUi(L"Restarting",L"Starting version "+installed,UpdateProgressPixels);bool restarted=RestartTrackedApplication(launched,installed);CloseUpdateUi();if(!restarted){launchResult=2;CloseLaunched(launched);MessageBoxW(nullptr,L"The update was installed, but the application could not be shut down and restarted. Start it again from the launcher.",cfg.displayName.c_str(),MB_ICONERROR);}}
            else{if(result==UpdateResult::Failed)ShowUpdateFailureUi(updateError,true);CloseUpdateUi();}}
    }
    std::wstring active;if(CurrentValid(active)){Cleanup(active);if(!updateFirst&&result==UpdateResult::Installed)RegisterWindowsIntegration(active);}if(!backgroundUpdate)StartRuntimeUpdate();ReleaseMutex(updateMutex);CloseHandle(gConfigurationMutex);gConfigurationMutex=nullptr;CloseHandle(updateMutex);return FinishLaunched(launched,launchResult);
}
#ifdef ONEKB_RUNTIME_ONLY
