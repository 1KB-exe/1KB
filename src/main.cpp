...
static void UpdateUiDownloadProgress(unsigned long long received,unsigned long long total){if(gUpdateUi.window&&total){int percent=(int)(received*100/total);if(percent>100)percent=100;SetUpdateUi(gFreshInstall?L"Downloading app":L"Downloading update",gUpdateProgressDetail,percent);}}
...
static bool PrepareAndActivate(const UpdateResponse& update,bool zip,std::wstring& error){
    ...
    SetUpdateUi(gFreshInstall?L"Downloading app":L"Downloading update",gUpdateProgressDetail,0);
    ...
    if(zip){SetUpdateUi(gFreshInstall?L"Installing app":L"Installing update",gUpdateProgressDetail,-1);bool extracted=ExtractZip(downloaded,dir);DeleteFileW(downloaded.c_str());if(!extracted){DeleteTree(dir);error=L"The downloaded ZIP file is invalid or could not be safely extracted. This launcher requires the tar.exe included with current Windows versions.";return false;}}
    SetUpdateUi(gFreshInstall?L"Installing app":L"Installing update",gUpdateProgressDetail,-1);std::wstring application=AppPath(update.version);if(application.empty()){DeleteTree(dir);error=L"The downloaded release does not have an unambiguous application executable.";return false;}
...
static int LauncherMain(){
    ...
    if(gUpdateUi.thread){if(result==UpdateResult::Installed)SetUpdateUi(hadInstalled?L"Update ready":L"Ready to launch",L"Version "+installed+L" is installed",100);else if(result==UpdateResult::Failed)SetUpdateUi(hadInstalled?L"Update couldn't be completed":L"Installation couldn't be completed",hadInstalled?L"Opening installed version "+existing:L"Please try again",0);}
