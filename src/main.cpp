...
// The bootstrap reuses this URL as its mutex name to avoid carrying another string.
...
static void ShowUpdateUi(const std::wstring& current){if(gUpdateUi.thread)return;gUpdateUi.status=L"Checking for updates";gUpdateUi.detail=current.empty()?L"Finding the latest build":L"Current build  "+current;gUpdateUi.progress=-1;gUpdateUi.ready=CreateEventW(nullptr,TRUE,FALSE,nullptr);if(!gUpdateUi.ready)return;gUpdateUi.thread=CreateThread(nullptr,0,UpdateUiThread,&gUpdateUi,0,nullptr);if(gUpdateUi.thread)WaitForSingleObject(gUpdateUi.ready,3000);CloseHandle(gUpdateUi.ready);gUpdateUi.ready=nullptr;}
...
static void UpdateUiDownloadProgress(unsigned long long received,unsigned long long total){if(gUpdateUi.window&&total){int percent=(int)(received*100/total);if(percent>100)percent=100;SetUpdateUi(L"Downloading update",gUpdateProgressDetail,percent);}}
...
static bool ValidLauncherRuntime(const std::wstring& path){DWORD type=0;WIN32_FILE_ATTRIBUTE_DATA data{};return IsGuiExecutable(path)&&GetBinaryTypeW(path.c_str(),&type)&&type==SCS_64BIT_BINARY&&GetFileAttributesExW(path.c_str(),GetFileExInfoStandard,&data)&&(((unsigned long long)data.nFileSizeHigh<<32)|data.nFileSizeLow)<=Limits::MaxApplicationSize;}
...
static bool LaunchInstalled(LaunchedProcess* launched){std::wstring current;if(!CurrentValid(current))return false;if(AppRunning())return true;return StartApplication(current,launched);}
...
static bool PrepareAndActivate(const UpdateResponse& update,bool zip,std::wstring& error){
    ...
    SetUpdateUi(L"Downloading update",gUpdateProgressDetail,0);
...
static LaunchedProcess* gRestartTarget=nullptr;
static bool gRestartStopped=false;
static bool StopTrackedApplication(LaunchedProcess& launched);
static UpdateResult CheckAndInstall(std::wstring& installed,std::wstring& error){installed.clear();error.clear();std::wstring current;bool haveCurrent=CurrentValid(current);UpdateResponse response;ResolveResult resolved=ResolveRemote(haveCurrent,response);if(resolved==ResolveResult::NotModified){CloseUpdateUi();return UpdateResult::NoUpdate;}if(resolved==ResolveResult::Failed){error=gLastHttpStatus?L"The application source server returned HTTP "+std::to_wstring(gLastHttpStatus)+L" while resolving the application manifest.":L"The application source URL could not be resolved to a valid application manifest.";return UpdateResult::Failed;}if(haveCurrent&&response.version==current){CommitApplicationValidators(response.validators);CloseUpdateUi();return UpdateResult::NoUpdate;}if(!gUpdateUi.thread&&(gRestartTarget||cfg.mode==UpdateMode::Yes))ShowUpdateUi(current);gUpdateProgressDetail=(current.empty()?L"New install":L"Build "+current)+L"  to  "+response.version;SetUpdateUi(L"Update found",gUpdateProgressDetail,0);if(gRestartTarget&&gRestartTarget->handle){SetUpdateUi(L"Closing the current app",gUpdateProgressDetail,-1);gRestartStopped=StopTrackedApplication(*gRestartTarget);}std::wstring object;if(!ValidateDownloadUrl(response.download,object)&&!(response.githubDownload&&GithubDownloadPath(response.download,object))){error=L"The application manifest's download URL is invalid.";return UpdateResult::Failed;}if(gLauncherOverlay.encryption==PayloadEncryption::Launcher&&!EndsWithInsensitive(response.download,L".1KB")){error=L"The private launcher encryption secret is missing or invalid.";return UpdateResult::Failed;}if(!PrepareAndActivate(response,EndsWithInsensitive(object,L".zip"),error))return UpdateResult::Failed;CommitApplicationValidators(response.validators);installed=response.version;return UpdateResult::Installed;}
static BOOL CALLBACK CloseProcessWindow(HWND window,LPARAM parameter){DWORD owner=0;GetWindowThreadProcessId(window,&owner);if(owner==(DWORD)parameter&&IsWindowVisible(window))PostMessageW(window,WM_CLOSE,0,0);return TRUE;}
static bool StopTrackedApplication(LaunchedProcess& launched){if(!launched.handle)return false;DWORD code=0;if(!GetExitCodeProcess(launched.handle,&code))return false;if(code==STILL_ACTIVE){EnumWindows(CloseProcessWindow,(LPARAM)launched.id);DWORD wait=WaitForSingleObject(launched.handle,Limits::GracefulRestartTimeoutMs);if(wait==WAIT_TIMEOUT){if(!TerminateProcess(launched.handle,0)||WaitForSingleObject(launched.handle,Limits::ForcedRestartTimeoutMs)!=WAIT_OBJECT_0)return false;}else if(wait!=WAIT_OBJECT_0)return false;}CloseHandle(launched.handle);launched.handle=nullptr;return true;}
...
bool GenerateDeploymentLauncher(const std::wstring& application,const std::wstring& output,const std::wstring& appId,bool removeIcon,PayloadEncryption encryption,const uint8_t secret[PayloadSecretBytes],std::wstring& error){AppIdentity identity;if(!ParseAppId(appId,identity)||identity.appId!=appId){error=L"The application ID is invalid.";return false;}if(encryption==PayloadEncryption::Launcher&&!secret){error=L"The private launcher encryption secret is unavailable.";return false;}WORD subsystem=0;if(!ExecutableSubsystem(application,subsystem)){error=L"The release application executable is invalid.";return false;}EmbeddedConfig config;config.appId=appId;config.encryption=encryption;if(secret)memcpy(config.secret,secret,PayloadSecretBytes);std::wstring operation;bool generated=GenerateLauncher(application,output,config,subsystem==IMAGE_SUBSYSTEM_WINDOWS_CUI,removeIcon?IconMode::None:IconMode::Small,operation);SecureZeroMemory(config.secret,PayloadSecretBytes);if(generated)return true;DWORD code=GetLastError();error=L"Could not "+operation+L" (Windows error "+std::to_wstring(code)+L").";return false;}
static int LauncherMain(){
    ...
    if(gUpdateUi.thread){if(result==UpdateResult::Installed)SetUpdateUi(L"Update ready",L"Build "+installed+L" is installed",100);else if(result==UpdateResult::Failed)SetUpdateUi(L"Update couldn't be completed",L"The installed build will be used",0);}
    ...
    if(updateFirst){if(hadInstalled&&result==UpdateResult::Failed)MessageBoxW(nullptr,(L"The installed application was started, but its update failed:\n\n"+updateError).c_str(),cfg.displayName.c_str(),MB_ICONWARNING);}
    else if(cfg.mode==UpdateMode::No&&StartBackgroundUpdate())backgroundUpdate=true;
    else{UpdateMode operationMode=cfg.mode;gRestartTarget=operationMode==UpdateMode::Restart?&launched:nullptr;gRestartStopped=false;result=CheckAndInstall(installed,updateError);gRestartTarget=nullptr;
        if(result==UpdateResult::Installed&&operationMode==UpdateMode::Restart){SetUpdateUi(L"Update ready",L"Starting build "+installed,100);bool restarted=gRestartStopped?StartApplication(installed,&launched):(launched.handle&&RestartTrackedApplication(launched,installed));CloseUpdateUi();if(!restarted)MessageBoxW(nullptr,L"The update was installed, but the exact application process could not be safely restarted.",cfg.displayName.c_str(),MB_ICONWARNING);}
        else{if(gRestartStopped)StartApplication(launched.version,&launched);CloseUpdateUi();}}
...
#ifdef ONEKB_RUNTIME_ONLY
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR arguments,int){DetectPrivateWorkerMode();if(gRuntimeApply)return RuntimeApplyMain();RuntimeStartupCleanup();
#ifdef ONEKB_RUNTIME_TESTS
