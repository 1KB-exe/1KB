...
static bool ParseIni(const std::vector<char>& bytes,IniValues& values){bool parsed=RuntimeSupport::ParseKeyValues(bytes,Limits::MaxUpdateConfigurationSize,[&](const std::string& key,const std::wstring& value){if(key=="version"){if(values.version||!ValidVersion(value))return false;values.version=true;values.versionValue=value;}else if(key=="download"){if(values.download)return false;values.download=true;values.downloadValue=value;}else if(key=="updates"){if(values.mode||!ParseMode(value,values.modeValue))return false;values.mode=true;}else return false;return true;});return parsed&&values.version&&values.download;}
...
static int LauncherMain(){
    ...
    if(!updateFirst){
        if(cfg.mode==UpdateMode::No&&StartBackgroundUpdate())backgroundUpdate=true;
            ...
            if(result==UpdateResult::Installed&&operationMode==UpdateMode::Restart){SetUpdateUi(L"Restarting",L"Starting version "+installed,100);bool restarted=launched.handle&&RestartTrackedApplication(launched,installed);CloseUpdateUi();if(!restarted){launchResult=2;CloseLaunched(launched);MessageBoxW(nullptr,L"The update was installed, but the application could not be shut down and restarted. Start it again from the launcher.",cfg.displayName.c_str(),MB_ICONERROR);}}
            else{if(result==UpdateResult::Failed)ShowUpdateFailureUi(updateError,true);CloseUpdateUi();}}
    }
    std::wstring active;if(CurrentValid(active))Cleanup(active);if(!backgroundUpdate)StartRuntimeUpdate();ReleaseMutex(updateMutex);CloseHandle(gConfigurationMutex);gConfigurationMutex=nullptr;CloseHandle(updateMutex);return FinishLaunched(launched,launchResult);
}
#ifdef ONEKB_RUNTIME_ONLY
