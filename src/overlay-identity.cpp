...
bool EncodeOverlayIdentity(const std::wstring& identity, const uint8_t* secret, size_t secretBytes,
    ...
    constexpr size_t oneKBBodyCharacters = std::size(OneKBIdentity) - 1 - 3;
    if (kind == GitHubKind && identity.compare(0, std::size(OneKBIdentity) - 1, OneKBIdentity) == 0 &&
        (identity.size() == std::size(OneKBIdentity) - 1 || identity[std::size(OneKBIdentity) - 1] == L'#'))
        for (size_t i = 0; i < oneKBBodyCharacters; ++i) wireIdentity[i] = static_cast<wchar_t>(towlower(wireIdentity[i]));
...
bool DecodeOverlayIdentity(const uint8_t* data, size_t dataBytes, size_t secretBytes,
    ...
    if (kind == GitHubKind && !_wcsnicmp(decoded.identity.c_str(), OneKBIdentity, std::size(OneKBIdentity) - 1) &&
        (decoded.identity.size() == std::size(OneKBIdentity) - 1 || decoded.identity[std::size(OneKBIdentity) - 1] == L'#'))
        decoded.identity.replace(0, std::size(OneKBIdentity) - 1, OneKBIdentity);
    if (!Canonical(decoded.identity, validator)) return false;
