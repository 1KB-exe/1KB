...
Before body encoding, `gh:`, `url:https://`, or `url:http://` is removed. GitHub bodies use the frozen `github-owner-reference-huffman` codec: canonical Huffman coding plus compact repository references into the owner. URL bodies use the frozen trained Huffman/token/copy codec; `#` is a direct structural symbol, and `1kb` is the only product-related codec token. Display casing is encoded as ordinary URL data only when a case-sensitive URL contains it.
