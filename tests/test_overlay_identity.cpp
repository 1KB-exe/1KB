...
static bool Validate(const std::wstring& value,std::wstring& normalized){
    normalized=value;
    if(value.rfind(L"gh:",0)==0){std::wstring body=value.substr(3);size_t slash=body.find(L'/');if(body.rfind(L"gh:",0)==0||slash==std::wstring::npos||!slash||slash+1>=body.size()||body.find(L'/',slash+1)!=std::wstring::npos)return false;for(size_t i=3;i<normalized.size();++i)normalized[i]=(wchar_t)towlower(normalized[i]);if(!_wcsnicmp(normalized.c_str(),OverlayIdentity::OneKBIdentity,wcslen(OverlayIdentity::OneKBIdentity))&&(normalized.size()==wcslen(OverlayIdentity::OneKBIdentity)||normalized[wcslen(OverlayIdentity::OneKBIdentity)]==L'#'))normalized.replace(0,wcslen(OverlayIdentity::OneKBIdentity),OverlayIdentity::OneKBIdentity);return true;}
...
int wmain(){
    ...
    CHECK(Encode(L"url:https://x/1KB.ini",wire)&&wire.size()==lowerTokenSize&&Decode(wire,decoded)&&decoded==L"url:https://x/1KB.ini");
